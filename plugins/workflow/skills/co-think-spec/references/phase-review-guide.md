...
Launch a `spec-reviewer` subagent via `Agent` with `subagent_type: "spec-reviewer"`. Include the scope, file paths, and report output path per `review-report.md` (in this references directory) in the prompt.
