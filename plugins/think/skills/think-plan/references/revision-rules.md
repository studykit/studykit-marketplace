...
4. **Reflecting risk assessment findings** — after applying confirmed risks from a `risk-assessor` report to the Risk Assessment section
5. **Session close** — at the end of an iteration (as part of the session close procedure)
6. **Finalization** — when setting `status: final`
...
3. If reflecting a review or risk assessment, also add the report filename to `reflected_files`
