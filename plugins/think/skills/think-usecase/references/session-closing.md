...
Launch `Agent(subagent_type: "think:usecase-explorer")`. Pass:
- `a4/` absolute path
...
Mark "Review: validate all UCs" `in_progress`. Launch `Agent(subagent_type: "think:usecase-reviewer")`. Pass:
- `a4/` absolute path
...
2. Append a `## Log` entry to the review item: `YYYY-MM-DD — resolved by editing [[<target path>]]`.
...
**Dismiss** — set `status: dismissed`, `updated: <today>`, append `## Log` entry with the reason.
...
Suggest the next step: `/think:think-arch` once the UC set and domain model feel stable, or `/think:think-usecase iterate` to pick up remaining review items later.

## Commit

Commit timing is user-driven: after each significant batch (e.g., after review-items walk, after wiki close guard). Recommend `/think:handoff` as the session-end commit path so the resumption handoff file records the shape of the session. Never commit from the main session without the user's say-so.
