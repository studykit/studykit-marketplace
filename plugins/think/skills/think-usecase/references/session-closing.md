...
> 4. Record open items and commit
>
> Ready to proceed?"
...
1. **Pre-checkpoint** — write all pending confirmed content to the working file and **increment `revision`**. Mark "Save progress" as `completed`.
...
8. Mark "Review" as `completed`. Mark "Record open items and commit" as `in_progress`. **Scan for Open Items** — review all sections for incomplete or unclear items:
...
12. **Offer GitHub Issues** — ask: "Would you like to create GitHub Issues for the use cases?"
    ...
    - If no → skip.
13. Mark "Record open items and commit" as `completed`. **Report** — show the user the current state and Open Items for next time. Suggest: "To design the architecture from these use cases, run `/think:think-arch <file_path>`."
