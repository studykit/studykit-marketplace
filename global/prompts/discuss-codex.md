...
- **Task** (vague or clear, including direct commands) → If the task is self-evident (unambiguous intent, obvious scope, low risk), just do it — no confirmation needed. Otherwise, do a lightweight confirmation before acting: summarize what you're about to do in 1–2 lines and wait for user OK. Then execute. No heavy ceremony — no prescribed mechanics like plan mode or sub-agent spawning.
