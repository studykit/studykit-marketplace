---
topic: "co-think-spec 이후 cross-cutting 스킬 설계 논의"
date: 2026-03-24
---
# Discussion: co-think-spec 산출물의 다음 단계

## Context
co-think-spec 산출물(Functional Spec)이 다음 단계로 어떻게 연결되는지에 대한 논의. spec 이후 솔루션 영역으로 바로 넘어갈지, 문제 영역을 더 구체화할지가 핵심 질문.

## Discussion Journey

### 1. Spec 산출물의 성격 확인
- spec은 story를 한 단계 구체화한 것
- story(왜, 어떤 가치) → spec(무엇을 해야 하는가 — 행동, 입출력, 에러)
...
### 6. 별도 스킬로 분리하는 이유
실제 사람의 사고에서는 spec↔concept을 오가는 게 자연스럽지만 AI 스킬로는 분리가 적합:
- 컨텍스트 집중 — 두 모드를 오가면 대화가 산만해짐
- 산출물 분리 — 성격이 다른 문서
- 입력/출력 명확성 — spec → concept 흐름이 깔끔
- 추상화 가드레일 — 스킬이 경계를 강제
...
- **frontmatter**: topic, date, source (spec 파일 wikilinks)
...
### 8. Spec 피드백 루프
1. co-think-concept → TODO 목록 생성, spec 리뷰 제안
2. 사용자가 co-think-concept-spec 실행 (concept 문서를 입력으로)
3. TODO 항목별 interview → spec FR 보완
4. 처리한 TODO에 체크 + 설명 기록 (concept-spec 스킬이 concept 문서에 marking)
5. TODO 체크 확인은 사용자 판단

### 9. co-think-concept-spec 스킬 분리
co-think-spec과 co-think-concept-spec은 목적과 대화 흐름이 다름:
- co-think-spec: story → 새로 spec 작성. story별 순차 진행
- co-think-concept-spec: concept feedback → 기존 spec 보완. TODO 항목별 진행

이름: `co-think-concept-spec` — concept에서 출발해서 spec으로 가는 흐름이 이름에 드러남.
...
→ FR에서 도출 가능한 시스템 경계는 concept에서 다루고, 외부 환경(인프라, 보안 등)만 별도로.

## Decisions

### 스킬 이름과 파이프라인
- `co-think-concept` — FR에서 도메인 개념 추출 (conceptual modeling)
- `co-think-concept-spec` — concept 피드백 기반으로 spec 보완
- 파이프라인: story → spec ↔ concept → architecture → implementation
...
- FR에서 도출 가능한 시스템 간 관계는 concept에서 다룸

## TODOs
- [ ] co-think-concept-spec 스킬 설계 및 생성
