...
2. **[#9][9]. Spawn a dedicated child session** — When the main session encounters a sub-problem, I want to spawn a child session with the interactive prompt loaded, so I can have specialized conversations in a dedicated session.
3. **[#10][10]. Automated information exchange between sessions** — When a child session is running, I want hooks to automatically exchange information between sessions, so I can keep the main session aware of child session progress.
4. **[#12][12]. Child session result file accessible to main session** — When I finish a child session, I want the result file path automatically registered with the main session, so I can reference the output directly.
5. **[#13][13]. Child session conversation history investigation** — When I want to review a past child session, I want the main session to read that session's history via a sub-agent, so I can understand the reasoning behind the output.
6. **[#14][14]. User controls session termination** — When I'm in a child session, I want the LLM to suggest wrapping up when appropriate while leaving the final decision to me, so I can keep exploring as long as I need. *(Already covered by [#16][16] — conversation-first behavioral layer includes session termination rules.)*
7. **[#15][15]. Skill injection at child session startup** — When I create a child session with a specific skill, I want that skill injected at startup, so I can start in the right structured dialogue mode.
...
### [#18][18]. Child session result delivery on termination
[status:: final]
> Story: [#10][10], [#12][12]
...
### [#19][19]. Child session conversation history investigation
[status:: final]
> Story: [#13][13]
...
**Dependencies:** [#17][17] (session.json with transcript_path)
...
[9]: https://github.com/studykit/studykit-plugins/issues/9
[10]: https://github.com/studykit/studykit-plugins/issues/10
[12]: https://github.com/studykit/studykit-plugins/issues/12
[13]: https://github.com/studykit/studykit-plugins/issues/13
[14]: https://github.com/studykit/studykit-plugins/issues/14
[15]: https://github.com/studykit/studykit-plugins/issues/15
[16]: https://github.com/studykit/studykit-plugins/issues/16
[17]: https://github.com/studykit/studykit-plugins/issues/17
[18]: https://github.com/studykit/studykit-plugins/issues/18
[19]: https://github.com/studykit/studykit-plugins/issues/19
