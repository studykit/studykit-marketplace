...
- **Scope split**: Story [#7][7] defines the interactive prompt's conversational attitude (standalone behavioral layer). Stories [#8][8]–[#15][15] define the **agent orchestrator** — the main session's ability to spawn, manage, and collect results from child sessions.

## Job Stories

### [#7][7]. Conversation-first attitude at session start
...
### [#8][8]. Spawn & manage child sessions *(parent)*
#### [#9][9]. Spawn a dedicated child session
...
#### [#10][10]. Automated information exchange between sessions
...
### [#11][11]. Child session results *(parent)*
#### [#12][12]. Child session result file accessible to main session
...
#### [#13][13]. Child session conversation history investigation
...
### [#14][14]. User controls session termination
...
### [#15][15]. Skill injection at child session startup
...
### Dependencies
- **[#9][9] → [#10][10]**: Spawning a child session ([#9][9]) must exist before information exchange ([#10][10]) is meaningful
- **[#9][9] → [#12][12]**: Spawning ([#9][9]) is a prerequisite for result file registration ([#12][12])
- **[#9][9] → [#13][13]**: Spawning ([#9][9]) is a prerequisite for history investigation ([#13][13])
- **[#9][9] → [#15][15]**: Spawning ([#9][9]) is a prerequisite for skill injection ([#15][15])

### Reinforcement
- **[#7][7] + [#14][14]**: Conversation-first attitude ([#7][7]) and user-controlled termination ([#14][14]) together ensure the LLM never rushes — neither at the start nor at the end
- **[#12][12] + [#13][13]**: File access ([#12][12]) and history investigation ([#13][13]) together give the main session full visibility into child session outputs
- **[#10][10] + [#12][12]**: Hook-based exchange ([#10][10]) enables automatic result registration ([#12][12])

### Groups
- **Conversational attitude**: [#7][7], [#14][14] — define how the LLM behaves within a session (interactive prompt scope)
- **Agent orchestrator – session lifecycle**: [#9][9], [#10][10], [#15][15] — define how sessions are created and configured
- **Agent orchestrator – session results**: [#12][12], [#13][13] — define how outputs flow back to the main session
...
[11]: https://github.com/studykit/studykit-plugins/issues/11
